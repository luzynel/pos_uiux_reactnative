import {StyleSheet,Text,TouchableOpacity,View,SafeAreaView,ScrollView,FlatList,TextInput,ActivityIndicator} from 'react-native';
...
const PendingOrders = () => {
  ...
  const poData = ({item}: {item: orderList}) => (
    <TouchableOpacity onPress={() => onPressNavigate('PendingOrdersView')}>
      ...
      <View style={GlobalStyles.framesWrapper}>
        ...
        <View style={GlobalStyles.frameMain}>
          <View style={[{flexDirection: 'row', alignItems: 'center',gap: 10,padding: 10,backgroundColor: color.light}]}>
            <IconItem itype={'AntDesign'} iname={'calendar'} isize={28} icolor={color.primary} />
            <View style={[CustomerInfoStyles.inputField, {width: 200,flex:0,}]}>
              <TextInput style={[CustomerInfoStyles.inputTextField]} placeholder="Today" placeholderTextColor={color.shadow} defaultValue='Today' />
            </View>
